...
function TableFooter({ className, ...props }: React.ComponentProps<'tfoot'>) {
  return (
    <tfoot
      ...
      className={cn(
        'bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 font-medium [&>tr]:last:border-b-0',
        className
      )}
      {...props}
    />
...
function TableRow({ className, ...props }: React.ComponentProps<'tr'>) {
  return (
    <tr
      data-slot="table-row"
      className={cn('hover:bg-muted/30 border-b border-border transition-colors', className)}
      {...props}
    />
  )
}
